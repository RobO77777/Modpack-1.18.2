onEvent('recipes', event => {
	...
	//create
		...
		event.shapeless('create:andesite_alloy', ['kubejs:cement', 'minecraft:andesite', Item.of('ceramicbucket:ceramic_bucket', '{Fluid:{Amount:1000,FluidName:"minecraft:lava"}}')])
		event.shapeless('kubejs:cement', ['minecraft:sand', 'minecraft:gravel', Item.of('ceramicbucket:ceramic_bucket', '{Fluid:{Amount:1000,FluidName:"minecraft:water"}}')])
	...
	//mekanism
		...
		event.recipes.createCrushing('mekanism:dust_copper', ['minecraft:copper_ingot'])
		
		event.recipes.createMixing('kubejs:crushed_bronze_ore', ['create:crushed_raw_copper', 'create:crushed_raw_tin']).superheated()
	...
	//Autres crafts
		...
		event.recipes.createMixing('minecraft:blaze_rod', ['#forge:rods/all_metal', 'minecraft:blaze_powder', 'minecraft:glowstone_dust'])
		event.shaped('minecraft:charcoal', [
			'CCC',
			   'CCC',
			  'CCC'
	...
	event.remove({output: 'ars_nouveau:worn_notebook'})
...
const MANA_AMOUNT = 1000;
const EMPTY_BUCKET = "minecraft:bucket";
const MANA_BUCKET = "kubejs:magical_primary_energy_bucket";
const MANA_JAR = "ars_nouveau:mana_jar";
const CREATIVE_MANA_JAR = "ars_nouveau:creative_mana_jar";

const isManaJar = block => block.id == CREATIVE_MANA_JAR || block.id == MANA_JAR;

onEvent("block.right_click", event => {
  const { item, block, player, server } = event;

  if (!(server && isManaJar(block))) {
    return;
  }

  const entity = block.getEntity();
  const currentMana = entity.getCurrentMana();
  const maxMana = entity.getMaxMana();
	
  if (item.id == MANA_BUCKET && currentMana < maxMana) {
    event.cancel();

    entity.addMana(MANA_AMOUNT);
    player.setMainHandItem(EMPTY_BUCKET);
  }

  if (item.id == EMPTY_BUCKET && currentMana >= MANA_AMOUNT) {
    event.cancel();

    entity.removeMana(MANA_AMOUNT);
    player.setMainHandItem(MANA_BUCKET);
  }
});

onEvent("item.right_click", event => {
  const { item, player } = event;
  const { block } = player.rayTrace(player.reachDistance);

  if (item.id == MANA_BUCKET && isManaJar(block)) {
    event.cancel();
  }
});